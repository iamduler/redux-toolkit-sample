...
export default function CreatePost() {
  ...
  const postId = useSelector((state: RootState) => state.blogState.postId);
  const { data } = useGetPostQuery(postId, { skip: !postId }); // skip: nếu postId là '', thì không gọi API
  const [updatePost, updatePostResult] = useUpdatePostMutation();

  // Khi data thay đổi, thì setFormData với data
  useEffect(() => {
    if (data) {
      setFormData(data);
    }
